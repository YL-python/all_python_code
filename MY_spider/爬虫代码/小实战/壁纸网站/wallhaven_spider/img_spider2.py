...
URL = 'https://wallhaven.cc/search?categories={}&purity=100&topRange={}&sorting={}&order=desc&page=@'
...
def request_page(url):
    try:
        ...
        if response.status_code != 404:
            return etree.HTML(response.content.decode("utf-8")).xpath(XPATH_STR)
        else:
            return None
    ...
    for sorting in sortingList:
        for topRange in topRangeList:
            try:
                ...
                print(e)
            url = URL.format(categories, topRange, sorting)
